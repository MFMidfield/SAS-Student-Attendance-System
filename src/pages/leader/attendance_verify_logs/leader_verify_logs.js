...
export function initLeaderVerifyLogs(imageLogo, imageBander) {
    ...
    const openNoteModal = (item) => {
        ...
        if (noteVerifiedBy) {
            ...
            if (verifier) {
                noteVerifiedBy.textContent = `${verifier.firstname || ''} ${verifier.lastname || ''}`.trim() || '-';
            } else {
        ...
        if (noteVerifiedAt) {
            if (item.verified_at) {
                const vDate = new Date(item.verified_at);
                noteVerifiedAt.textContent = vDate.toLocaleDateString('th-TH', {
                    year: 'numeric', month: 'short', day: 'numeric',
                    hour: '2-digit', minute: '2-digit'
                });
            } else {
                noteVerifiedAt.textContent = '-';
            }
        }
    ...
    const renderLogs = () => {
        ...
        if (filtered.length === 0) {
            logList.innerHTML = `<div class="text-center py-10 opacity-30 font-bold uppercase italic tracking-tighter">
                ${searchTerm ? 'No results matching your search.' : 'No verified logs found.'}
            </div>`;
            return;
        }
            ...
            row.innerHTML = `
                ...
                <div class="flex-1 flex flex-col min-w-0">
                    ...
                    <div class="px-3 py-1 text-[11px] font-bold opacity-60 italic flex items-center gap-2">
                        ...
                        ${item.is_verified ? '<span class="text-[#219653] not-italic text-[9px]">✓ VERIFIED</span>' : ''}
                    </div>
                </div>
                <button class="view-note-btn w-12 shrink-0 flex flex-col items-center justify-center gap-1 border-l-2 border-[#1E1E1E] ${hasNote ? 'bg-[#F2C00F]' : 'bg-white'} hover:bg-[#F2C00F] transition-colors" title="ดูรายละเอียด">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                    <span class="text-[7px] font-black">DETAIL</span>
                </button>
            `;
    ...
    // --- Fetch Verified Attendance from Database ---
    const fetchLogs = async () => {
        ...
        // ดึงจาก attendance_verify พร้อม join profiles ของ verified_by
        let query = supabase
            .from('attendance_verify')
            .select('*, verifier:profiles!attendance_verify_verified_by_fkey(firstname, lastname, role)');
        ...
        if (roomFilter) {
            if (userRole === 'admin') {
                ...
                // ถ้าเป็น Teacher ไม่ต้องโชว์ตัวกรองห้อง เพราะเห็นแค่ห้องตัวเองอยู่แล้ว
    ...
    if (dateFilter) dateFilter.addEventListener('change', fetchLogs);
    if (searchFilter) searchFilter.addEventListener('input', renderLogs);
    if (backBtn) backBtn.addEventListener('click', () => { window.location.hash = '#leader-dashboard'; });
