...
export function initSetting(role, userAvatar, imageBander) {
    ...
    const fetchUserInfo = async () => {
        ...
        if (user) {
            ...
            if (profile) {
                if (studentRoleElem) studentRoleElem.textContent = profile.role || role;
                if (infoRole) infoRole.textContent = profile.role || role;
            }
        }
    };
    fetchUserInfo();
    ...
    // Back to dashboard dynamically based on role
    if (backBtn) {
