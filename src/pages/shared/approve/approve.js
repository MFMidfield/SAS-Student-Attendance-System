...
export function initApprove(role, imageLogo, imageBander) {
    ...
    const openNoteModal = (item) => {
        ...
        if (noteStudentId) noteStudentId.textContent = (item.stu_id_record || '-') + ' | No: ' + (item.roll_no_record ?? '-');
    ...
    const renderLogs = () => {
        ...
        filtered.forEach(item => {
            ...
            if (!isPending && item.verifier) {
                verifierInfo = `${item.verifier.firstname} (${item.verifier.role})`;
            }
            ...
            row.innerHTML = `
                ...
                <div class="flex-1 flex flex-col min-w-0">
                    ...
                    <div class="px-3 py-1 text-[11px] font-bold opacity-60 italic truncate">
                        ID: ${escapeHTML(item.stu_id_record || '-')} <span class="opacity-40">|</span> No: ${escapeHTML(item.roll_no_record ?? '-')} <span class="opacity-40">|</span> ${escapeHTML(detailText)} <span class="opacity-40">|</span> ${escapeHTML(verifierInfo)} <span class="opacity-40">|</span> ${escapeHTML(item.verifier?.firstname || '')} (${escapeHTML(item.verifier?.role || '')})
                    </div>
                </div>
    ...
    if (backBtn) backBtn.addEventListener('click', () => { window.location.hash = `#${role}-dashboard`; });
            ...
            items.forEach((item, idx) => {
                ...
                row.innerHTML = `
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-[9px] font-bold opacity-50 uppercase tracking-tighter">ID: ${escapeHTML(item.stu_id_record || '-')} · เลขที่: ${escapeHTML(item.roll_no_record ?? '-')} · ROOM: ${escapeHTML(item.class_id_record || '-')}</p>
                    </div>
